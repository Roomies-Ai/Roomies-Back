...
export class AuthController {
    ...
    @Post('/google')
    async googleLogin(@Body() body: { token: string }, @Res({ passthrough: true }) res: Response, @Req() req: Request) {
    ...
    @Post('/logout')
    async logout(@Body() body: { email: string, password: string }, @Res({ passthrough: true }) res: Response, @Req() req: Request) {
        const result = await this.authService.logout(req, res);
        return result;
    }

    @Post('/refresh')
    async refresh(@Body() body: { email: string, password: string }, @Res({ passthrough: true }) res: Response, @Req() req: Request) {
