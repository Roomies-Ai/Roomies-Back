...
import { getNextOccurrenceDate, buildInstanceFromTemplate, RecurrenceRule } from './recurrence.helper';
...
describe('getNextOccurrenceDate', () => {
  describe('DAILY', () => {
    it('returns the next day by default', () => {
      ...
      const next = getNextOccurrenceDate(rule, date('2024-01-01T10:00:00Z'));
    ...
    it('respects interval > 1', () => {
      ...
      const next = getNextOccurrenceDate(rule, date('2024-01-01T00:00:00'));
      expect(next!.getDate()).toBe(4);
    });

    it('applies timeOfDay to the result', () => {
      const rule: RecurrenceRule = { frequency: 'DAILY', interval: 1, timeOfDay: '08:30' };
      const next = getNextOccurrenceDate(rule, date('2024-01-01T00:00:00'));
    ...
    it('returns null when endDate has passed', () => {
      const rule: RecurrenceRule = { frequency: 'DAILY', interval: 1, endDate: '2024-01-01' };
      const next = getNextOccurrenceDate(rule, date('2024-01-01T12:00:00'));
      expect(next).toBeNull();
    });

    it('returns null when next occurrence exceeds endDate', () => {
      const rule: RecurrenceRule = { frequency: 'DAILY', interval: 5, endDate: '2024-01-04' };
      const next = getNextOccurrenceDate(rule, date('2024-01-01T00:00:00'));
      expect(next).toBeNull();
    });
  });
  ...
  describe('WEEKLY', () => {
    it('finds the next Sunday when daysOfWeek=[0]', () => {
      // 2024-01-01 is a Monday (day 1), so next Sunday is 2024-01-07
      const rule: RecurrenceRule = { frequency: 'WEEKLY', interval: 1, daysOfWeek: [0] };
      const next = getNextOccurrenceDate(rule, date('2024-01-01T00:00:00'));
      expect(next).not.toBeNull();
    ...
    it('finds the closest matching day when multiple daysOfWeek provided', () => {
      // 2024-01-01 is Monday. Next occurrence of Wed(3) or Fri(5) → Wed 2024-01-03
      const rule: RecurrenceRule = { frequency: 'WEEKLY', interval: 1, daysOfWeek: [3, 5] };
      const next = getNextOccurrenceDate(rule, date('2024-01-01T00:00:00'));
      expect(next!.getDay()).toBe(3); // Wednesday
    });

    it('wraps across week boundary', () => {
      // 2024-01-06 is Saturday (day 6). Next Mon (day 1) → 2024-01-08
      const rule: RecurrenceRule = { frequency: 'WEEKLY', interval: 1, daysOfWeek: [1] };
      const next = getNextOccurrenceDate(rule, date('2024-01-06T00:00:00'));
    ...
    it('returns null when next occurrence is past endDate', () => {
      const rule: RecurrenceRule = {
        ...
        interval: 1,
        daysOfWeek: [0],
        endDate: '2024-01-05', // Sunday 2024-01-07 is after this
      };
      const next = getNextOccurrenceDate(rule, date('2024-01-01T00:00:00'));
      expect(next).toBeNull();
    });
  ...
  describe('MONTHLY', () => {
    it('returns the same day next month', () => {
      ...
      const next = getNextOccurrenceDate(rule, date('2024-01-15T00:00:00'));
    ...
    it('returns null when past endDate', () => {
      const rule: RecurrenceRule = { frequency: 'MONTHLY', interval: 1, endDate: '2024-01-31' };
      const next = getNextOccurrenceDate(rule, date('2024-01-15T00:00:00'));
      expect(next).toBeNull();
    });
  });
...
describe('buildInstanceFromTemplate', () => {
  const template = {
    ...
    taskType: { id: 'type-1' },
    household: { id: 'hh-1' },
    recurrenceRule: { frequency: 'WEEKLY' as const, interval: 1, daysOfWeek: [0] },
    googleCalendarEventId: 'old-event',
  };

  it('copies core fields from the template', () => {
    const nextDue = new Date('2024-01-07T08:00:00');
